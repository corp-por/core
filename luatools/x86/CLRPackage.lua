...
function CLRPackage(assemblyName, packageName)
  ...
  local ok, err = pcall(load_assembly,assemblyName)			-- Make sure our assembly is loaded
  print("Load .NET Assembly '"..assemblyName.."' OK="..tostring(ok)..", err="..tostring(err))
...
function import (assemblyName, packageName)
    ...
    print(".NET Assembly import:"..tostring(assemblyName).."=>"..tostring(packageName))
